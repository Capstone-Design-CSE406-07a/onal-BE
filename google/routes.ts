  ...
  passport.authenticate('google', (err: Error | null, user : IUser | false, info: { message: string } | undefined) => {
    ...
    if (!user) {
      if (info && info.message === 'USER_NOT_FOUND') {
        return res.redirect(`http://localhost:5173/login?status=USER_NOT_FOUND`);
      }
      return res.redirect(`http://localhost:5173/login?status=LOGIN_FAIL`);
    }
    ...
    req.login(user, (loginErr) => {
      ...
      return res.redirect(`http://localhost:5173/login?status=LOGIN_SUCCESS`);
    });
  })(req, res, next);
});
